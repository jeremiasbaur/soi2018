...
f = open('pipeline-sub2-attempt5.txt', 'r')
...
for i in range(testcases):
    ...
    while count_pumps != max_pumps:
        ...
        if l >= r:
            #print("l bigger or equal than r")
            j = 1
            looking_from = 0
            pump_true = [False for x in range(pumps_count)]
            impossible = False
            ...
            while required_count != max_pumps and j != pumps_count:
                #print("kkk",required_count, max_pumps, l, r,prev_mid)
                if not pump_true[j]:
                    pump_true[j] = True
                    required_count += 1
                j+=1
            #print(max_pumps,required_count,pump_stations,"\nlast", pump_true, prev_mid)
